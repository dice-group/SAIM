...
//
	...
	public static List<String> subclassesOf(String clazz,String endpoint, String graph)
	{
		...
		QueryExecution qexec = graph==null?sparqlService(endpoint, sparqlQuery):sparqlService(endpoint, sparqlQuery,graph);
		ResultSet rs = qexec.execSelect();
		List<String> classes = new LinkedList<>();
		while(rs.hasNext()) {classes.add(rs.next().getResource("?class").toString());}
		return classes;
	}
